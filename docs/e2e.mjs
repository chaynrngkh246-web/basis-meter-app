...
console.log('\n— the TOU sheet reads at a ratio —')
await step('the Excel seed puts x1,000 on the TOU consumption codes', async () => {
  await page.click('[data-act="go"][data-v="set"]')
  await page.waitForSelector('[data-act="excel"]', { timeout: 8000 })
  page.once('dialog', (d) => d.accept())
  await page.click('[data-act="excel"]')
  await page.waitForTimeout(900)
  const mea = page.locator('.card').filter({ hasText: 'MEA มิเตอร์การไฟฟ้า' }).first()
  await mea.locator('[data-act="popen"]').click()
  await page.waitForTimeout(300)
  const facs = await mea.locator('[data-act="mfac"]').inputValues?.() ??
    await mea.locator('[data-act="mfac"]').evaluateAll((els) => els.map((e) => e.value))
  // the three consumption codes read x1,000; the four peaks have no ratio box
  if (facs.filter((f) => f === '1000').length !== 3)
    throw new Error('expected 3 codes at x1,000, got ' + facs.join('|'))
})
await step('a dial moving 2 is reported as 2,000 kWh', async () => {
  await page.click('[data-act="go"][data-v="home"]')
  await page.waitForSelector('[data-act="rec"]', { timeout: 8000 })
  await page.locator('[data-act="rec"]').filter({ hasText: 'MEA มิเตอร์การไฟฟ้า' }).first().click()
  await page.waitForSelector('.reg input', { timeout: 8000 })
  const fc = page.waitForEvent('filechooser')
  await page.click('[data-act="shot"]')
  ;(await fc).setFiles([PHOTO])
  await page.waitForSelector('.shot img', { timeout: 20000 })
  await page.locator('.reg input').nth(0).fill('2472')
  await page.click('[data-act="save"]')
  await page.waitForSelector('.daycard', { timeout: 10000 })
  ...
  await page.locator('[data-act="rec"]').filter({ hasText: 'MEA มิเตอร์การไฟฟ้า' }).first().click()
  await page.waitForSelector('.reg input', { timeout: 8000 })
  await page.locator('.reg input').nth(0).fill('2474')
  await page.waitForTimeout(250)
  const used = await page.locator('.mfoot .u b').first().innerText()
  if (used !== '2,000') throw new Error('expected 2 x 1,000 = 2,000, got ' + used)
})
await step('the ratio can be changed from settings', async () => {
  ...
  await mea.locator('[data-act="popen"]').click()
  await page.waitForTimeout(300)
  const box = mea.locator('[data-act="mfac"]').first()
  await box.fill('1')
  await box.dispatchEvent('change')
  await page.waitForTimeout(600)
  const after = await mea.locator('[data-act="mfac"]').first().inputValue()
  if (after !== '1') throw new Error('ratio did not stick: ' + after)
})
