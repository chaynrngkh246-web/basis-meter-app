...
> **มี 2 เวอร์ชันในโปรเจกต์นี้**
>
> 1. **เวอร์ชันพร้อมใช้ทันที (แนะนำ)** — `artifact/meter-log.html`
>    เว็บแอปไฟล์เดียว ใช้ฐานข้อมูลในตัวของ Claude Artifacts ไม่ต้องตั้งค่าเซิร์ฟเวอร์
>    ไม่ต้องสมัคร Firebase เผยแพร่แล้วได้ลิงก์ใช้งานเลย
>    ทดสอบอัตโนมัติด้วย `node artifact/e2e.mjs` (ต้องมี playwright)
>    **ข้อจำกัด:** คนที่เปิดลิงก์ได้ต้องเป็นสมาชิกในองค์กร claude.ai เดียวกับเจ้าของแอป
>
> 2. **เวอร์ชัน Firebase** — โค้ดใน `src/` (React + TypeScript)
>    ใช้เมื่อต้องการให้ใครก็เปิดลิงก์ได้โดยไม่ต้องมีบัญชี claude.ai
>    ต้องตั้งค่าโปรเจกต์ Firebase เองตามคู่มือด้านล่าง
